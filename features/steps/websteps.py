...
@Then('Set SMS Advance Setting of Validity "{vv}" Period "{vp}" Encoding "{ev}" Priority "{priority}" MessageUDH "{mudh}" Overwrite "{ow}"')
...
@Then('create Event for metric "{metric}" entitytype "{entitytype}" profiletype "{profiletype}" profilename "{profilename}" monitorlevel "{monitorlevel}" monitorperiod "{monitorperiod}" thresholdtype "{thresholdtype}" thresholdvalue "{thresholdvalue}" deviceaction "{deviceaction}"')
def step_impl(context, metric, entitytype, profiletype, profilename, monitorlevel,monitorperiod,thresholdtype,thresholdvalue,deviceaction):
      context.execute_steps("""
      ...
      if ("Radius" not in metric) and ("SMS Delivery" not in metric) and ("Packet Data Volume (Bytes In Only)" not in metric):
          context.execute_steps("""
      ...
      if (monitorperiod == "Set hourly range"):
          ...
          """)
      if (metric == "Time In State" or metric == "Voice Duration" or metric =="Period since First transition into Active-Live state"):
          context.execute_steps("""
